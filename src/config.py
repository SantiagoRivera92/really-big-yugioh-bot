...
class Config:
	...
	def setDefaultLeagueChannel(self, channelId:int, serverId: int):
		config = self.getConfigForServer(serverId)
		config[DEFAULT_LEAGUE_CHANNEL_KEY] = channelId
		self.saveConfigForServer(config, serverId)
		return OperationResult(True, Strings.MESSAGE_DEFAULT_LEAGUE_CHANNEL_SET)

	def getDefaultLeagueChannel(self, currentChannelId:int, serverId: int):
		config = self.getConfigForServer(serverId)
